...
DOUBLE_WIDTH = 1000
BLACK_LIST = []
TQDM_COLORS = ["RED", "GREEN", "YELLOW", "BLUE", "MAGENTA", "CYAN", "WHITE"]
VALID_COLLECTIONS = [
    "dbmultiverse",
    "namekseijin",
    "dbm-colors",
...
def download_image(number, collection="dbmultiverse"):
    ...
    if not os.path.isfile(jpg_path) and not os.path.isfile(png_path):
        ...
        try:
            ...
            xpath_element = html.xpath("//*[@id='balloonsimg']")
            if xpath_element:
                element = xpath_element[0]
                img_url = get_img_url_from_element(element, collection)

                if img_url:
                    full_url = urljoin(PARENT_URL, img_url)
                    img_format = parse_qs(urlparse(full_url).query).get("ext", [None])[
                        0
                    ]
                    img_path = get_full_path(
                        "DBM_", number, img_format, collection=collection
                    )
...
def download_images(max_books, collection="dbmultiverse"):
    ...
    with ThreadPoolExecutor() as executor:
        list(
            tqdm(
                executor.map(
                    lambda number: download_image(number, collection),
                    range(int(max_books)),
                ),
                total=int(max_books),
    ...
    with ThreadPoolExecutor() as executor:
        list(
            tqdm(
                ...
                total=len(tasks),
                desc=f"Converting {collection} to PDF",
                colour=TQDM_COLORS[VALID_COLLECTIONS.index(collection)],
            )
        )
...
def merge_pdfs(max_books, collection):
    ...
    for num in tqdm(
        range(max_books),
        desc=f"Merging {collection} PDFs",
        colour=TQDM_COLORS[VALID_COLLECTIONS.index(collection)],
    ):
        pdf_path = get_full_path(
            "DBM_", num, "pdf", folder=PDF_PATH, collection=collection
        )
...
def get_latest_chapter(collection):
    ...
    chapter_numbers = [
        int(link.text_content().strip())
        for link in chapter_links
        if link.text_content().strip().isdigit()
    ]
...
def process_collection(collection):
    """Process and download all chapters for a specific collection."""
...
def main():
    ...
    if collection == "all":
        ...
        if collection not in VALID_COLLECTIONS:
            print(
                f"Invalid collection: {collection}. Please use one of the following: {', '.join(VALID_COLLECTIONS)}"
            )
            return
        process_collection(collection)
    print(f"Process finished for {collection}")
